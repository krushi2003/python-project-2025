...
class Game:
    ...
    def render_background(self):
        bg = pygame.image.load("resources/background.jpg")  # Load the new nature background
        bg = pygame.transform.scale(bg, (1000, 800))  # Resize to fit the screen
        self.surface.blit(bg, (0, 0))  # Draw the background on the screen
    ...
    def play(self):
        ...
        for i in range(self.snake.length):
            if self.is_collision(self.snake.x[i], self.snake.y[i], self.apple.x, self.apple.y):
                ...
                if self.snake.length % 5 == 0:  # Increase level every 5 apples
                    self.level += 1
                    self.speed = max(0.1, self.speed - 0.02)  # Increase speed
